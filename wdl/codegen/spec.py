...
PATIENT = [
    ...
    over("ancestry_pred", "String?", "peddy | RunPeddy"),
    over("ancestry_prob", "Float?", "peddy | RunPeddy"),
    over("ancestry_background_pca_table", "File?", "peddy | RunPeddy"),
    over("ancestry_pca_plot", "File?", "peddy | RunPeddy"),
    over("modeled_segments", "File?"),
    # PhylogicNDT is run n_replicates times; each field holds one entry per replicate.
    over("phylogic_sif_file", "Array[File]?", "PhylogicNDT | one per replicate"),
    over("phylogic_report", "Array[File]?", "PhylogicNDT | one per replicate"),
    over("phylogic_ccfs_cnvs", "Array[File]?", "PhylogicNDT | one per replicate"),
    over("phylogic_ccfs_snvs", "Array[File]?", "PhylogicNDT | one per replicate"),
    over("phylogic_constrained_ccf", "Array[File]?", "PhylogicNDT | one per replicate"),
    over("phylogic_cluster_ccfs", "Array[File]?", "PhylogicNDT | one per replicate"),
    over("phylogic_build_tree_posteriors", "Array[File]?", "PhylogicNDT | one per replicate"),
    over("phylogic_growth_rates", "Array[File]?", "PhylogicNDT | one per replicate"),
    over("phylogic_growth_rate_plot", "Array[File]?", "PhylogicNDT | one per replicate"),
    over("phylogic_timing_report", "Array[File]?", "PhylogicNDT | one per replicate"),
    over("phylogic_timing_wgd_supporting_events", "Array[File]?", "PhylogicNDT | one per replicate"),
    over("phylogic_timing_graph", "Array[File]?", "PhylogicNDT | one per replicate"),
    over("phylogic_timing_comparison", "Array[File]?", "PhylogicNDT | one per replicate"),
    over("phylogic_timing_table", "Array[File]?", "PhylogicNDT | one per replicate"),
]
