...
workflow DefineRuntimeCollection {
    input {
        ...
        # Compute resources are optimized for WES and most cohort WGS cases.
        # This policy switch deliberately trades cost for a higher chance of
        # completing unusual cases without repeated tuning attempts.
        Boolean JUST_RUN_IM_WILLING_TO_PAY = false
        Int mem_BIG_MACHINE = 8192

        # Task-specific controls retained for empirically variable runtimes.
    ...
    # Fixed internal runtime defaults. Memory is in MB and time is in minutes.
    # These are deliberately not workflow inputs: changing them requires a
    # reviewed workflow revision (or an HPC-specific source revision).
    ...
    # HPC scheduler limits for the otherwise user-tunable task families.
    ...
    Int time_mutect1_total = 2880  # 2 d
    ...
    # Clonal analysis workflow, including fixed ABSOLUTE settings.
    ...
    Runtime create_mutect2_panel = {
        ...
        "max_retries": max_retries,
        "cpu": cpu,
        "machine_mem": mem_create_mutect2_panel + mem_create_mutect2_panel_overhead,
